...
cleanup_eips() {
    log "INFO" "🧹 Cleaning up orphaned Elastic IPs..."
    
    # Get orphaned EIPs
    ORPHANED_EIPS=$(aws ec2 describe-addresses --region "$REGION" \
        --query 'Addresses[?AssociationId==null && InstanceId==null].AllocationId' \
        --output text)
    
    if [ -n "$ORPHANED_EIPS" ] && [ "$ORPHANED_EIPS" != "None" ]; then
        log "INFO" "Found orphaned EIPs: $ORPHANED_EIPS"
        ...
        log "INFO" "No orphaned EIPs found"
    fi
}
